...
uint32_t cache_lookup_dm(struct cache_st *csp, uint64_t addr) {
	...
	// calculating the word address by right shifting input address by 2
	uint64_t addr_word = addr >> 2;
	// calculating the block index and base address
   	b_index = addr_word % csp->block_size;	// remainder of dividing the word address by block size
    b_base = addr_word - b_index;	// subtracting block index from word address
    uint64_t b_base_byte = b_base << 2;  // converting block base address back to byte address by left shifting by 2
    ...
    if (slot->valid && (slot->tag == tag)) {
		...
		// populate the cache block with data from memory
        for (int i = 0; i < csp->block_size; i++) {
            slot->block[i] = *(((uint32_t*)b_base_byte) + i);
        }

    	// get data from updated block
        data = slot->block[b_index];
    }
...
uint32_t cache_lookup_sa(struct cache_st *csp, uint64_t addr) {
    ...
    uint64_t addr_word = addr >> 2;
	// calculating the block index and base address
   	uint64_t b_index = addr_word % csp->block_size;	// remainder of dividing the word address by block size
    uint64_t b_base = addr_word - b_index;	// subtracting block index from word address
    uint64_t b_base_byte = b_base << 2;  // converting block base address back to byte address by left shifting by 2
    ...
    if (!slot_found) {
        if (slot_invalid) {
            // uses the first invalid slot if available
            slot = slot_invalid;
            ...
            // implementing LRU replacement strategy
            uint64_t earliest_time = csp->refs + 1;
            struct cache_slot_st *oldest_slot = NULL;
            
            // find the LRU slot
            for (int i = 0; i < csp->ways; i++) {
                slot = &csp->slots[set_base + i];
                if (slot->timestamp < earliest_time) {
                    earliest_time = slot->timestamp;
                    oldest_slot = slot;
                }
            }
    
            if (oldest_slot) {
                slot = oldest_slot; // use the LRU slot
                verbose("  cache tag (%X) miss for set %d tag %X addr %X (evict address %X)\n",
                        slot->tag, set_index, tag, addr, 
                        ((slot->tag << (csp->index_bits + csp->block_bits + 2)) | (set_index << (csp->block_bits + 2))));
                csp->misses += 1;
                // Miss due to tag collision is a "hot" miss
                csp->misses_hot += 1;
            }
    ...
    slot->timestamp = csp->refs; // updating the timestamp to mark current access
    return value;
